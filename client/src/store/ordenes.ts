...
const useOrdenes = create<UserStore>()(
    devtools((set) => ({
      ...
      putOrden: async (body, token) => {
        ...
        const { data } = await axios.put('carteleriamanna.up.railway.app/api/orden', body, { headers: { "x-access-token": token} });
      ...
      getOrdenesDate: async (date1, date2, token) => {
        ...
        try{
          set({ loading: true}) 
          const { data } = await axios.get(`carteleriamanna.up.railway.app/api/ordenes/ordenesbydate?date1=${date1}&date2=${date2}`,{ headers: { "x-access-token": token} }
          )
      ...
      set({ success: true, error: false });
       try{ 
        const { data } = await axios.post('carteleriamanna.up.railway.app/api/ordenes/create', body, { headers: { "x-access-token": token} });
      }catch (error) {
      ...
      getOrdenes: async (headers) => {
        try{
          set({ loading: true}) 
          const { data } = await axios.get('carteleriamanna.up.railway.app/api/ordeness', headers )
          set((state) => ({ ordenes2: (state.ordenes2 = data) }));
        }catch(error){
          console.log(error)
        }
      ...
      getOrdenesAll: async (token, page, limit) => {
        try{
          set({ loading: true}) 
          const { data } = await axios.get(`carteleriamanna.up.railway.app/api/ordeness/all?page=${page}&limit=${limit}`,
      ...
      deleteOrdenes: async (params, headers, idUser)=>{
        set({ success: true, error: false });
        const { data } = await axios.delete(`carteleriamanna.up.railway.app/api/ordene/${params}?idUser=${idUser}`,   headers);
        set({ success: true, error: false });  
      },
      closeModal: () => {
        //set({ error: false})
