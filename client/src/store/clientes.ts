...
const useClients = create<ClientStore>()(
	devtools((set) => ({
		...
		putClients: async (body, token) => {
		  ...
		  set({ success: true})
		  set({ loading: true}) 
			const { data } = await axios.put('https://carteleriamanna.up.railway.app/api/clientes', body, { headers: { "x-access-token": token} });
			set({ success: false})
		...
		getClients: async (headers) => {
			try {
				...
				const { data } = await axios.get(
					"https://carteleriamanna.up.railway.app/api/clientes",
					headers
				);
		...
		addClient: async (body) => {
			try {
				await axios.post(
					"https://carteleriamanna.up.railway.app/api/clientes/create",
					body
				);
		...
		deleteClients: async (params, headers)=>{
			set({ loading: true}) 
			const { data } = await axios.delete(`https://carteleriamanna.up.railway.app/api/clientes/${params}`,   headers);
			set({ loading: false})  
		  },
		closeModal: () => {
